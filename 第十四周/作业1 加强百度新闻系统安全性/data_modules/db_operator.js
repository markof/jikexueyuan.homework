...
/**
 * 安全增强
 * htmlencode主要是为了规避在标题等不需要html的输入上将非法输入进行转码。
 * santitize-html则是对需要html输入的html进行白名单过滤,这里是在默认白名单的基础上增加了自定义的白名单。
 */
...
function getNewsByPage(pageNumber, callback) {
    !isNaN(Number(pageNumber)) && newsFinder({}, pageNumber, callback);
}
    
//获取新闻，通过新闻id
function getNewsByID(id, callback) {
    !isNaN(Number(id)) && newsFinder({ 'id': id }, 1, callback);
}
    
//获取新闻，通过类别信息
function getNewsByCat(id, pageNumber, callback) {
    !isNaN(Number(id)) && newsFinder({ 'category_id': id }, pageNumber, callback);
}
    
//获取新闻，通过关键字
function getNewsByKeywords(keywords, pageNumber, callback) {
    newsFinder({ 'title': orm.like("%" + htmlencoder.htmlEncode(keywords) + "%") }, pageNumber, callback);
}
...
function addNews(news_title, news_content, news_cat_id, callback) {
    let title = htmlencoder.htmlEncode(news_title);
    let content = sanitizeHtml(news_content,whitelist);

    isNewsExist(title, content, news_cat_id, function (err, isExist) {
        if (err) callback(err);
        else {
            if (isExist) {
...
function addCategory(cat_title, callback) {
    let title = htmlencoder.htmlEncode(cat_title);
    isCategoryExist(title, function (err, isExist) {
        if (err) callback(err);
        else {
            if (isExist) {
...
function modifyCategory(cat_id, cat_title, callback) {
    ...
    db_module.category().get(cat_id, function (cat_get_err, category) {
        if (cat_get_err) callback(cat_get_err);
            ...
            category.save({ 'title': title }, function (cat_save_err) {
...
function isNewsExist(news_title, news_content, news_cat_id, callback) {
    // 此处的title和content已经做过安全检查，无需再处理。
    let title = news_title;
    let content = news_content;
    
    db_module.news().exists({ 'title': title, 'content': content, 'category_id': news_cat_id }, function (err, isExist) {
        if (err) callback(err);
...
function isCategoryExist(cat_title, callback) {
    // 此处cat_title已经转义，无需再转义。
    let title = cat_title;
    
    db_module.category().exists({ 'title': title }, function (err, isExist) {
        if (err) callback(err);
        else callback(err, isExist);
    });
}
